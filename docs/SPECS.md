...
`emit-c` writes `<stem>.c` or `<stem>.cu` plus the runtime headers needed by that program to `out/<stem>/` by default. Pass `--out-dir DIR` to choose the output directory.
